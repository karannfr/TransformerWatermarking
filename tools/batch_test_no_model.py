#!/usr/bin/env python3
"""Batch test tool (no importance model).

For each image in --input-dir (up to --limit), embed a random payload using a random key,
then immediately extract and validate the extracted payload equals the original. Also
compute PSNR/SSIM between original and watermarked images and write results to CSV/JSON.

Usage:
python tools\batch_test_no_model.py --input-dir ./images --output-dir ./results_no_model --ecc-symbols 64
"""
...
def run_cmd(cmd, timeout=120) -> Tuple[int, str, str, float]:
    start = time.time()
...
def single_test(image_path: Path, out_dir: Path, ecc_symbols: int, message_bytes: int) -> dict:
    results = {}
...
def main() -> None:
    ...
    for img in images:
        ...
        r = single_test(img, out, args.ecc_symbols, args.message_bytes)
